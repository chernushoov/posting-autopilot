...
def register_routes(app):
    ...
    def login():
        if request.method == "POST":
            ...
            # regression: detached-instance after db.close() — snapshot the
            # primitives we need before closing the session, then read.
            if user:
                user_id = user.id
        ...
        return render_template("login.html", demo_email=Config.DEMO_EMAIL, demo_password=Config.DEMO_PASSWORD)
    ...
    def settings():
        ...
        if request.method == "POST":
            # regression: detached-instance after settings save — DO NOT read
            # user.full_name after db.close(); capture into a primitive first.
        ...
        db.close()
        return render_template("settings.html", user=user, settings_row=settings_row, telegram_token_configured=bool(Config.TELEGRAM_BOT_TOKEN))
